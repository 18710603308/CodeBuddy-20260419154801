...
import { getSMBROM } from '../smbRom'

// 超级马里奥兄弟 - Super Mario Bros FC
const SuperMario: React.FC = () => {
    ...
    try {
      ...
      const nes = new NES({
        ...
        onStatusUpdate: (status: string) => {
          ...
          if (status.includes('World')) {
            const match = status.match(/World:\s*(\d+)/)
            if (match) setWorld(parseInt(match[1]))
          }
    ...
    try {
      ...
      if (nesWrapper?.nes) {
        if (romData.length < 100) {
          setRomError('ROM文件未加载或无效，长度: ' + romData.length)
          setGameState('start')
          return
        }

        // 确保是Uint8Array格式
        const uint8Array = new Uint8Array(romData)
        console.log('Loading SMB ROM, size:', uint8Array.length, 'first bytes:', Array.from(uint8Array.slice(0, 16)))
        
        nesWrapper.nes.loadROM(uint8Array)
        setGameState('playing')
        keysRef.current = {}
        startGameLoop()
      } else {
        setRomError('NES模拟器未初始化')
        setGameState('start')
      }
    } catch (error) {
      console.error('ROM load error:', error)
      setRomError('ROM加载失败: ' + (error as Error).message)
      setGameState('start')
    }
  ...
  const saveGame = useCallback((slot: number) => {
    ...
    try {
      ...
      localStorage.setItem(`smb-save-${slot}`, JSON.stringify(Array.from(state)))
  ...
  const loadGame = useCallback((slot: number) => {
    ...
    try {
      const saved = localStorage.getItem(`smb-save-${slot}`)
      if (saved) {
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              <div className={`hidden sm:flex gap-3 ${theme.text}`}>
                ...
                <span className="text-sm">世界: <b className="text-blue-400">{world}-{level}</b></span>
              </div>
      ...
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-60px)] px-4 pb-8">
        ...
        {!isMobile && (
          <>
            ...
            <p className={`${theme.textSubtle} text-sm mb-4`}>超级马里奥兄弟 FC复刻版</p>
          </>
        )}
        {isMobile && (
          <>
            ...
            <p className={`${theme.textSubtle} text-xs mb-2`}>超级马里奥兄弟 FC复刻版</p>
          </>
        )}
          ...
          {gameState === 'start' && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/90 rounded-lg">
              <div className="text-center">
                ...
                <h2 className={`text-3xl font-bold mb-4 ${theme.text}`}>SUPER MARIO BROS</h2>
                <p className={`${theme.textSubtle} text-sm mb-4`}>超级马里奥兄弟</p>
                ...
                <div className={`${theme.textSubtle} text-xs mb-6 space-y-1`}>
                  <p>← → ↑ ↓ 移动 | <span className="text-green-400">Z</span> 跳跃 | Enter 开始</p>
                  <p>方向键移动, Z跳跃</p>
                </div>
                ...
                <div key={slot} className="text-center">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className={`px-3 py-1 rounded text-sm ${isDark ? 'bg-red-600/50 hover:bg-red-600' : 'bg-red-400'} text-white`}
                    >
                      保存
                    </button>
                    <button
        ...
        {!isMobile && (
          <div className="mt-6 text-center max-w-md">
            ...
            <p className={`${theme.textSubtle} text-xs leading-relaxed`}>
              键盘：← → ↑ ↓ 移动 | <span className="text-green-400">Z</span> 跳跃 | Enter 开始<br/>
              踩敌人、吃蘑菇变强，救出公主！
            </p>
          </div>
        )}
